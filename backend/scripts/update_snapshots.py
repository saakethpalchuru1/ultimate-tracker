...
Pipeline:
  1. in_run_window() gate (default Sat/Sun 08:30-20:00 America/Chicago)
  2. Fetch USAU schedule HTML (plain httpx, server-rendered page)
  3. parse_schedule_html -> List[Game]
  4. pipeline.build_snapshots_from_games -> 4 JSON payloads
  5. github_push.push_files -> commits to GitHub Contents API

The scraper and pipeline are deliberately separated; scripts/push_fixture.py
shares the same pipeline but feeds it canned games for local testing.
"""
